...
SET ANSI_NULLS ON
GO
CREATE procedure [amy].[rpt_fb_seatareapotential_revenuemodel_byseatarea] (@ticketyear  int)
 AS
...
select fm.Side, fm.[Level], [Seating Option],Category, SeatAreaName,ticketpriceperseat,  annualperticket,
--[Total Seats], 
--[Annual Contribution],
dtl.[Total_Seats],
dtl.[Unsold_Seats], 
dtl.[Sold_Seats],
--[Total Annual] [Funding Model Annual], 
dtl.[Potential_Annual], 
dtl.[Net_Annual_Expected],  
dtl.[Gross_Annual_Expected] ,
dtl.[credit_applied],
Ticket_Amount_potential ,
Ticket_Amount_actual ,
dtl.[Potential_Annual]  +Ticket_Amount_potential  Revenue_Potential,
dtl.[Net_Annual_Expected]+ Ticket_Amount_actual Revenue_Forcast_based_on_actuals 
from 
(
select  fundingreportid, SeatAreaName,
...
sum(Ticket_Amount_potential) Ticket_Amount_potential ,
sum(Ticket_Amount_actual) Ticket_Amount_actual ,
ticketpriceperseat,  annualperticket
/* Ticket_Amount_potential ,
 Ticket_Amount_actual ,
...
[Season - Donor], [Season - Donor (E/EC)], [Season - Comp], [Season - Public], 
[Season - Non-Renewable],
 [Individual - Donor], [Inventory], [Student], [Discount], [OtherSold], 
 --[Students], [To Be Allocated], [Visiting Team], [Student Groups], 
--[Season], [Corps], [Problem], [Recruits], [Student Athletes], [Player Comp], [Donor Individual Sales], [Public Individual Sales], 
--[Mini Pack], [Wheelchair], [Groups 5], [Kill], [Special Needs], [Ath Dept], [High School Coaches], [Consignment], [TicketOfficeHolds], [OtherUnsold]
[StudentsAvailable], [StudentsComps],
...
sum (case when isnull(tixseatcurrentstatus,0)<>2 and  tixsysseatstatusdesc = 'Student Athletes'  then 1 else 0
end ) [Student Athletes],
...
WHERE 1=1 
 ...
 left join (    select seatareaname, seatareaid,  sum( adjusted_annual)   Annual_Expected , sum(annual_expected ) original_annual_expected , 
...
seatareaname, seatareaid , premium_ind
 ...
 ) fmsum group by fundingreportid,  seatareaname  ,ticketpriceperseat,  annualperticket
 ) dtl ,
