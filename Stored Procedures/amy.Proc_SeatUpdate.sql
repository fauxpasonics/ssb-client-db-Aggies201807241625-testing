...
--and isnull(j.ordernumber , 0) = isnull(t.ordernumber , 0)
--and isnull(j.tixeventid , 0)  = isnull(t.tixeventid  , 0)
--and isnull(j.tixseatgroupid , '') = isnull(t.tixseatgroupid  , '')
--and isnull(j.tixseatgrouppricelevel , 0)  = isnull(t.tixseatgrouppricelevel  ,0)
--and isnull(j.tixseatofferid , '')   = isnull(t.tixseatofferid   , '')
 ) existingrecord, 
 ordernumber, ordergroupdate, ordergrouplastupdated ,  tixseatgroupid ,tixseatgrouppricelevel, tixseatofferid ,ticketvalue,ticketpaid 
from  (
select * from 
(select   accountnumber, categoryname, tixeventlookupid, tixeventtitleshort, tixeventid, [year], seatpricecode, seatsection,
seatrow, seatseat, tixseatid, tixsyspricecode, paid, sent, paidpercent, schpercent, ordergroupbottomlinegrandtotal, ordergrouptotalpaymentsonhold,
ordergrouptotalpaymentscleared, tixseatlastupdate, ordernumber, ordergroupdate, ordergrouplastupdated, tixseatgroupid ,tixseatgrouppricelevel, tixseatofferid ,ticketvalue,ticketpaid 
...
select * from 
(select   accountnumber, categoryname, tixeventlookupid, tixeventtitleshort, tixeventid, [year], seatpricecode, seatsection,
seatrow, seatseat, tixseatid, tixsyspricecode,  tixseatgroupid ,tixseatgrouppricelevel, tixseatofferid ,ticketvalue,ticketpaid 
...
ROW_NUMBER() OVER(ORDER BY accountnumber,cap_percent_owe_todate desc, seatsection,seatrow,seatseat) AS CancelRowNumber  from amy.seatdetail_individual_history where accountnumber in (
select accountnumber from (select accountnumber, 
sum(case when cancel_date >= getdate()-2  and lineal_transfer_release_ind is null and relocation_release_ind is null then 1  else 0 end) cancels,
sum(case when create_date >= getdate()-2  and lineal_transfer_received_ind is null and  relocation_start_ind is null and cancel_ind is  null and relocation_release_ind is null  and seatsection not in ('Request Pending')  then 1  else 0 end) creates
from  amy.seatdetail_individual_history  where tixeventlookupid = @currenttixeventlookupid and
((cancel_date >= getdate()-2  and lineal_transfer_release_ind is null and relocation_release_ind is null
) or 
(create_date >= getdate()-2 and lineal_transfer_received_ind is null and  relocation_start_ind is null and cancel_ind is  null and relocation_release_ind is null and seatsection not in ('Request Pending') 
)
)
group by accountnumber) T where cancels >0 and creates >0 ) 
and tixeventlookupid = @currenttixeventlookupid and (cancel_date >= getdate()-2  and lineal_transfer_release_ind is null   and relocation_release_ind is null and seatpricecode not in ('Suite SRO')
) ) cancel,
...
amy.seatdetail_individual_history ct , amy.VenueSections  ks ,        
               amy.SeatRegion sr
          where  ct.tixeventlookupid =  @currenttixeventlookupid and  ks.sporttype  = 'FB' and ct.seatsection   = ks.sectionnum 
...
select accountnumber from (select accountnumber, sum(case when cancel_date >= getdate()-2  and lineal_transfer_release_ind is null  and relocation_release_ind is null and seatpricecode not in ('Suite SRO') then 1  else 0 end) cancels,
sum(case when create_date >=  getdate()-2 and lineal_transfer_received_ind is null  and  relocation_start_ind is null  and cancel_ind is  null and relocation_release_ind is null  and seatsection not in ('Request Pending') then 1  else 0 end ) creates
from  amy.seatdetail_individual_history  where tixeventlookupid = @currenttixeventlookupid and
((cancel_date >= getdate()-2  and lineal_transfer_release_ind is null  and relocation_release_ind is null and seatpricecode not in ('Suite SRO')
) or (create_date >= getdate()-2 and lineal_transfer_received_ind is null  and  relocation_start_ind is null  and cancel_ind is  null and relocation_release_ind is null  and seatsection not in ('Request Pending') 
))
group by accountnumber) T where cancels >0 and creates >0 ) 
and tixeventlookupid = @currenttixeventlookupid and (create_date >= getdate()-2 and lineal_transfer_received_ind is null  and  relocation_start_ind is null  and cancel_ind is  null and relocation_release_ind is null  and seatsection not in ('Request Pending') 
)
...
begin 
         ...
         FETCH NEXT FROM TixCursor
              ...
              @ordergrouptotalpaymentscleared, @tixseatlastupdate, @existingrecord, @ordernumber,  @ordergroupdate, @ordergrouplastupdated, 
               @tixseatgroupid ,   @tixseatgrouppricelevel , @tixseatofferid , @ticketvalue ,   @ticketpaid
...
select 
case when renewal_ordernew = 1 and renewal_ticketnew = 0 and relocation_renewal_ticketnew = 0 and paid is not null then 1 else 0 end addonticket, 
  case when renewal_ordernew = 0 and renewal_ticketnew = 0 and paid is not null then 1 else 0 end newticketnew, 
  case when renewal_ordernew = 1 and (renewal_ticketnew = 1 or relocation_renewal_ticketnew = 1)  then 1 else 0 end renewalticketfinal,
a.* from (
...
and s1.seatsection =  s.seatsection and s1.seatrow = s.seatrow and s1.seatseat = s.seatseat
and s1.seatpricecode not like '%SRO%' and s.seatpricecode not like '%SRO%'),0) renewal_ticketnew,
...
and s1.tixeventlookupid = @previoustixeventlookupid  and s1.accountnumber = s2.accountnumber and 
 s1.seatsection =  s2.seatsection and s1.seatrow = s2.seatrow and s1.seatseat = s2.seatseat
 and s1.seatpricecode not like '%SRO%' and s2.seatpricecode not like '%SRO%'),0) relocation_renewal_ticketnew,
renewal_order, 
 renewal_ticket, 
new_ticket,
--and 
...
from seatdetail_individual_history s where tixeventlookupid = @currenttixeventlookupid   and cancel_ind is  null  --and accountnumber = 252557
...
then update
 ...
 merge seatdetail_individual_history r using (
select  seatdetail_individual_history_id, ordernumber, min( p.paymenttransactiondate) transdate
from seatdetail_individual_history s  
left join ods.VTXpayments p on s.ordernumber = p.ordergroupid
where tixeventlookupid = @currenttixeventlookupid   and cancel_ind is  null  --and accountnumber = 252557
and p.paymenttransactiondate <> '01/01/0001'
group by  seatdetail_individual_history_id, ordernumber
 ) tk 
on r.seatdetail_individual_history_id = tk.seatdetail_individual_history_id 
when matched 
then update
 set r.min_ticketpaymentdate= tk.transdate
     ;
end
GO
