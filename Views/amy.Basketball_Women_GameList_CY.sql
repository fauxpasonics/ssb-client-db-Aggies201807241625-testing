...
SET ANSI_NULLS ON
GO
CREATE VIEW [amy].[Basketball_Women_GameList_CY]
AS

SELECT e.tixeventid, e.tixeventlookupid, e.tixeventtitleshort,  e.tixeventstartdate
FROM ods.VTXtixevents e
WHERE e.tixeventlookupid LIKE 'B' + cast ((select substring(max(ticketyear),3,2)   from PriorityEvents where sporttype = 'BB-WB') as varchar)+  '-WB[0-9][0-9]'
