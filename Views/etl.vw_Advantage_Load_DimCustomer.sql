...
CREATE VIEW [etl].[vw_Advantage_Load_DimCustomer] AS 
(
...
--base set
  ...
  SELECT
                      ...
                      , con.ADNumber AS [ExtAttribute11] --decimal
			  ...
			  LEFT JOIN (
							...
							(
								...
								AND PK <> (SELECT MAX(ca1.PK) FROM dbo.ADVContactAddresses ca1 WHERE ca1.ContactID = ca.ContactID AND ca1.PrimaryAddress = 1)
							) a
							WHERE rownum = 1
					) ad2
              ...
              WHERE 1=1
			  --not all contacts have multiple primaries, and not all have a primary flag
			  AND (
					--they have one or more primaries
					(ad1.pk = (SELECT MAX(ad4.PK) FROM dbo.ADVContactAddresses ad4 WHERE ad4.ContactID = con.ContactID AND ad4.PrimaryAddress = 1))
				 
				 --they don't have a primary flagged
				 OR (ad1.pk = (SELECT MAX(ad5.PK) FROM dbo.ADVContactAddresses ad5
								WHERE ad5.ContactID = con.contactID
								AND ad5.PrimaryAddress = 0
								AND NOT EXISTS (SELECT ad6.pk FROM dbo.ADVContactAddresses ad6 WHERE ad6.PrimaryAddress = 1 AND ad6.ContactID = con.contactid)
								))
				--they don't have an address
				OR (NOT EXISTS(SELECT ad7.contactid FROM dbo.ADVContactAddresses ad7
				                  WHERE	ad7.ContactID = con.contactID))
				   )
