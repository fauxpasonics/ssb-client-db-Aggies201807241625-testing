...
CREATE PROCEDURE [etl].[Advantage_LoadDimCustomer] 
AS
BEGIN
...
USING (select * from [etl].[vw_Advantage_Load_DimCustomer]) as mySource
     ON
...
/* - not needed for Advantage

/***AddressOne***/
MERGE dbo.DimCustomer AS myTarget
USING (select * from ods.vw_Advantage_LoadDimCustomer) as mySource
     ON
		myTarget.SourceDB = mySource.SourceDB
		and myTarget.SourceSystem = mySource.SourceSystem
		and myTarget.SSID = mySource.SSID
