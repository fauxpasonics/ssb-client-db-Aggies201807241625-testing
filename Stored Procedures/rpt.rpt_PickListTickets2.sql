...
CREATE    PROCEDURE [rpt].[rpt_PickListTickets2]  (@EventID NUMERIC) WITH RECOMPILE  AS  
...
SELECT
	...
	--,hpp.curr_yr_cash_pts as prioritypoints
FROM (SELECT *, cast (tixseatpricecode AS NUMERIC) AS tixseatpricecodenumeric 
		 ,  cast (tixseatordergroupid AS NUMERIC) AS tixseatordergroupidnumeric  
				FROM  [ods].[VTXtixeventzoneseats] WHERE tixseatpricecode <> '' AND  tixseatordergroupid <> '' ) a
