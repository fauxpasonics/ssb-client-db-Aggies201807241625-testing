...
SET ANSI_NULLS ON
GO
--exec [amy].[proc_SeatRegionReport_table_forcast] 'FB','2017'
CREATE PROCEDURE [amy].[proc_SeatRegionReport_table_forcast] (@sporttype varchar(20) = 'FB', @ticketyear varchar(4)= '2016', @reporttype int =2 )
AS
...
Declare @tixeventid varchar(50)
DECLARE @forcastpercent numeric (1,1)
Declare @reporttixeventlookupid varchar(50)    ---depends on if forcasting or current year
Declare @reportticketyear varchar(4)  ---depends on if forcasting or current year

set @tixeventlookupid = (select tixeventlookupid from amy.PriorityEvents pe1 where sporttype = @sporttype and ticketyear = @ticketyear and tixeventid is not null);
set @tixeventid = (select tixeventid from amy.PriorityEvents pe1 where sporttype = @sporttype and ticketyear = @ticketyear and tixeventid is not null);

if @tixeventid is null
  begin
    set @reporttixeventlookupid = (select tixeventlookupid from amy.PriorityEvents pe1 where sporttype = @sporttype and ticketyear = @ticketyear-1);
    set @tixeventlookupid  = 'F' +  substring(@ticketyear,3,2)  +'-Season'
    set @forcastpercent = .2
    set @reportticketyear =@ticketyear-1
  end
else
  begin
    set @reporttixeventlookupid = @tixeventlookupid
    set @forcastpercent = 0
    set @reportticketyear =@ticketyear
  end
...
"Adv CAP Credit",
null "Advantage Adjusted Annual Pledge",
null "Adv Annual  Pledge",
null "Adv Annual Receipt",
null "Adv Annual Match Pledge",
null "Adv Annual Match Receipt",
null "Adv Annual Credit", 
...
null  --isnull(Annual,0) -  isnull("Advantage Adjusted Annual Pledge", 0)
annualdifference , 
isnull(CAP, 0)   - (isnull( "Adv CAP Receipt AMount",0) +isnull( "Adv CAP Match Receipt" ,0) +isnull(  "Adv CAP Credit",0) ) caprecdifference,
null --isnull(ANNUAL,0) - (isnull("Adv Annual Receipt",0) +isnull("Adv Annual Match Pledge",0)+ isnull("Adv Annual Credit", 0)+ isnull(AnnualScheduledPayments,0))
annualrecdifference ,
null AnnualScheduledPayments, 
...
amy.SeatAllocation SAlloc
         WHERE  Sarea.SeatAreaID = SAlloc.SeatAreaID
                ...
                AND transyear IN ( --@reportticketyear,
				'CAP')
...
select     sr.seatregionname, ct.seatregionid, cast(ct.accountnumber as int) adnumber,cap_amount, annual_amount
  from  seatdetail_individual_history ct,   
               amy.SeatRegion sr
          where  ct.tixeventlookupid =  @reporttixeventlookupid  and
...
select  seatregionid,
  ...
  ct.seatregionid,  cast(accountnumber as int)  adnumber, ct.ordernumber, ct.ordergroupbottomlinegrandtotal, ct.ordergrouptotalpaymentsonhold, ct.ordergrouptotalpaymentscleared,
    ...
    sum(case when seatpricecode in ('E','E Hold', 'E ADA','E ADA2' ,'Suite E') then  1 /*qty*/  else 0 end ) E, 
    sum(case when seatpricecode in ('EC','EC Hold', 'EC ADA','EC ADA2' ,'EC WC','Suite EC') then  1 /*qty*/  else 0 end) EC, 
    ...
    sum(case when seatpricecode like 'Faculty%'  and  seatpricecode not in ( 'Faculty&Staff (AD)' ) then  1 /*qty*/ else 0 end) Faculty, 
    sum(case when seatpricecode in ( 'DC' )then  1 /*qty*/ else 0 end) DC, 
    sum(case when seatpricecode  like  'Comp%' or  seatpricecode in ('Contract Purchase - Ann & Cap Donation Exempt') then  1 /*qty*/ else 0 end) Comp,
    sum(case when seatpricecode in ( 'UO','Suite UO' )then  1 /*qty*/ else 0 end) UO,
    sum( case when seatpricecode in ( 'UO-R' )then  1 /*qty*/ else 0 end) [UO-R], 
    sum(case when seatpricecode in ( '2015 Season Only (Non Renewable)','Season Only (Non Renewable)','Season Only DC (Non-Renewable)'  )then  1 /*qty*/ else 0 end) singleseason,  
    sum(1) ticketcount ,  
  --  min(ct.paidpercent) ticketpaidpercent  ,  
   -- min(ct.schpercent) ticketscheduledpercent,
    sum( case when  ct.CAP_Percent_Owe_ToDate+ (1*@forcastpercent) = 0.2 then  1 /*qty*/ else 0 end) CAP_20, 
    sum( case when  ct.CAP_Percent_Owe_ToDate+ (1*@forcastpercent) = 0.4 then  1 /*qty*/ else 0 end) CAP_40, 
    sum( case when  ct.CAP_Percent_Owe_ToDate+ (1*@forcastpercent) = 0.6 then  1 /*qty*/ else 0 end) CAP_60, 
    sum( case when  ct.CAP_Percent_Owe_ToDate+ (1*@forcastpercent) = 0.8 then  1 /*qty*/ else 0 end) CAP_80, 
    sum( case when  ct.CAP_Percent_Owe_ToDate+ (1*@forcastpercent) = 1  or ct.CAP_Percent_Owe_ToDate = 1 then  1 /*qty*/ else 0 end) CAP_100,
        sum( case when  ct.CAP_Percent_Owe_ToDate+ (1*@forcastpercent) not in (.2,.4,.6,.8,1) then  1 /*qty*/ else 0 end) CAP_Other,
    sum(sapg.CAP * (ct.CAP_Percent_Owe_ToDate+ (1*@forcastpercent) ) ) CAP_DUE  ,
     ...
     --   max(ct.ordernumber) ordernumber, count(distinct ordernumber) ordernumbercount, 
        ...
        from   amy.seatdetail_individual_history  ct, 
          ...
          where
          ct.tixeventlookupid =  @reporttixeventlookupid
          and py.sporttype =  @sporttype  and py.ticketyear =  @reportticketyear 
          and py.PriceCodeCode    = ct.seatpricecode        
          and  sapg.SeatAreaID  =  cast(ct.seatareaid as int)
          and sapg.pricegroupid = py.PriceGroupID  
          and sapg.sporttype =  @sporttype   and sapg.ticketyear =  @reportticketyear   
...
exec [amy].[proc_SUITE_table] @sporttype , @ticketyear 
exec amy.proc_SUITESEATREGION_table @sporttype , @ticketyear 
...
min_annual_receipt_date,
 capital_ada_credit_amount , Sporttype,  Ticketyear,  TixEventLookupID, updatedate,
 ticketpaidpercent  ,  
  ticketscheduledpercent,
 cap_20, CAP_40,CAP_60, CAP_80, CAP_100, CAP_OTHER, CAP_DUE, LinealTransfer, 
 CapStillDue, 
 ordernumber,
 seatlastupdated,
 vtxcusttype from amy.RPT_SUITESEATREGION_TB where sporttype = @sporttype  and ticketyear= @ticketyear
GO
