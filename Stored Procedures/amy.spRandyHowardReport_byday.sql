...
SET ANSI_NULLS ON
GO
create PROCEDURE [amy].[spRandyHowardReport_byday]
(
	@start_date	DATETIME,
	@end_date	DATETIME,
	@trans_year	char(4)
...
sum(case when isnull(sa.premium_ind,0) = 0 then  py.Annual else 0 end) nonpremium_annual_expected 
  ...
  left join seatarea sa on ct.seatareaid = sa.seatareaid
      ...
      py.sporttype =  'FB'  and py.ticketyear =  2017
       ...
       , advcontacttranslineitems l where c.contactid = h.contactid and  h.transid = l.transid and transyear = '2017' and transtype = 'Credit' and programid = 678
