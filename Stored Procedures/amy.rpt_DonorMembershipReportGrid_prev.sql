...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [amy].[rpt_DonorMembershipReportGrid_prev] (@startdate date = '01/01/2015', @enddate date = '12/31/2015')
...
--                         WHERE mm3.membershipname = '12th Man Foundation - All' and mm3.transyear =  @CurrentYear );
 ...
 Set @yearenddate = cast ('12/31/' + @CurrentYear AS DATE);               
 
 select MEMB_Code = 'TMF-ALL'


SELECT  t.adnumber, firstname, lastname, address1, address2, address3, city, state, zip, email, 
      case when ( YTD_CAP_Receipt + YTD_Annual_Receipt + YTD_Credit_Total >= 25) and  (enrolldate between @startdate and @enddate or previous_level is null)
...
case when (YTD_CAP_Receipt + YTD_Annual_Receipt + YTD_Credit_Total >= 25000 and Previous_YTD_CAP_Receipt + Previous_YTD_Annual_Receipt + Previous_YTD_Credit <25000)  then 'X' else null end  [Diamond 12th Man]     ,
          ...
          FROM advcontact c
               ...
               (SELECT h.contactid,
                       ...
                       SUM (
                          ...
                          END)   EnrollDate, 
                            max ( CASE
                             WHEN   (  transtype LIKE '%Receipt%' or transtype like '%Credit%') and h.PaymentType <> 'Transfer of Funds'
                             THEN
                               (CASE
                                          WHEN year (transdate) <  @CurrentYearDT
                                          THEN
                  ...
                  --select *
                   count ( distinct CASE
                             WHEN     transtype LIKE '%Receipt%' and ( isnull(transamount,0) +isnull(matchamount,0) >0) and h.PaymentType <> 'Transfer of Funds'
                       ...
                       INNER JOIN dbo.advContactTransLineItems l
                          ON     h.transid = l.transid
                             ...
                             AND (   h.TransYear =@CurrentYear
                                  OR (    h.TransYear = 'CAP'
                                      AND transdate  between @yearstartdate and  @yearenddate))
  ...
  ON     ytd_all.membid = mbrl.membid
                     AND ytd_all.YTD_CAP_Receipt + YTD_Annual_Receipt + ytd_all.YTD_Credit_Total BETWEEN mbrl.MinAmount
                                                                          AND mbrl.maxamount
               LEFT JOIN
               ( SELECT ml.membid,  ml.levelid,          ml.levelname,   ml.minamount,
                       isnull (min (ml2.minamount - .0001), 1000000000)
                          maxamount
                  FROM dbo.advMembershipLevels ml
                       LEFT JOIN dbo.advMembershipLevels ml2
