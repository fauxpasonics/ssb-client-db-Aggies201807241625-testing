...
SET ANSI_NULLS ON
GO
--exec [amy].[rpt_DonorProfileTransactionSummary_due] 21426
CREATE PROCEDURE [amy].[rpt_DonorProfileTransactionSummary_due]
@ADNumber int,
...
and p.programid not in (select programid from amy.SeatAllocation)
...
--and (matchinggift = 0 or matchinggift is null)
         ...
         and  (transyear  = 'CAP' or transyear >= cast( year( getdate())-1 as varchar))
