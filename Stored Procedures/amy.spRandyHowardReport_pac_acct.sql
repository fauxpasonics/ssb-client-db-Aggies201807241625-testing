...
--minus
--exec [amy].[spRandyHowardReport_pac]  '3/31/2017', '03/31/2017','2017'
create PROCEDURE [amy].[spRandyHowardReport_pac_acct]
(
...
sum(case when isnull(sa.premium_ind,0) = 0 then  py.Annual else 0 end) nonpremium_annual_expected 
  ...
  from amy.PacTranItem_alt_vw p  
    ...
    -- AND TransType = 'Cash Pledge'
		GROUP 	BY p.acct, p.allocationID, p.AllocationName 
		UNION
		SELECT 	p.acct, 
