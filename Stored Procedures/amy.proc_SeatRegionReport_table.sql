...
SET ANSI_NULLS ON
GO
--exec proc_SeatRegionReport_table 'FB', '2017', 2
CREATE PROCEDURE [amy].[proc_SeatRegionReport_table] (@sporttype varchar(20) = 'FB', @ticketyear varchar(4)= '2016', @reporttype int =2 )
AS
...
KFCScheduledPayments,
 ...
 ordernumber,
 seatlastupdated,
 (select CustomerType from amy.vtxcustomertypes where accountnumber = cast(adnumber as varchar)) vtxcusttype, null email
 from 
----------------------------------------------list
( select 
...
join ( select  distinct sporttype , programid from SeatAllocation sa 
...
WHERE resp_code IS NULL AND resp_text IS NULL  and t.transyear = 'CAP'
 AND t.contactid = advdata.ContactID 
 and sa1.SeatRegionID = alltickets.seatregionid 
 ),0)  
 + 
...
min_annual_receipt_date,
 ...
 ordernumber,
 seatlastupdated,
 vtxcusttype, null email from amy.RPT_SUITESEATREGION_TB where sporttype = @sporttype  and ticketyear= @ticketyear


 

UPDATE RPT_SEATREGION_TB
SET  email = rr.email
FROM (
    select distinct  tb.adnumber ,-- , sporttype, ticketyear --, seatregionid,   
 tt.email from amy.RPT_SEATREGION_TB  tb, amy.email_feed_vw  tt  where cast(tb.adnumber as varchar) = tt.accountnumber 
 and  sporttype = @sporttype  and ticketyear= @ticketyear ) rr
WHERE 
   RPT_SEATREGION_TB.adnumber = rr.adnumber and 
   RPT_SEATREGION_TB.sporttype =  @sporttype  and  RPT_SEATREGION_TB.ticketyear= @ticketyear 
...
left join (
 ...
 from amy.SeatArea Sarea,  amy.SeatAllocation SAlloc1  ---with suites this is an issue since seatareas map to same seatallocation
         WHERE     S
         area.SeatAreaID = SAlloc1.SeatAreaID and sarea.sporttype =  @sporttype ) salloc 
where l.ProgramID = SAlloc.ProgramID
AND c.contactid = h.contactid
AND h.TransID = l.TransID
AND p.ProgramID = l.ProgramID
