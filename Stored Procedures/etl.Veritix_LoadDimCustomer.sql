...
CREATE PROCEDURE [etl].[Veritix_LoadDimCustomer] 
AS
BEGIN
...
USING (select * from etl.vw_Veritix_Load_DimCustomer) as mySource
     ON
...
(
		SourceDB, SourceSystem, SourceSystemPriority, SSID, NameDirtyHash, NameIsCleanStatus, /*CustomerType, CustomerStatus, AccountType, AccountRep, CompanyName, Birthday, Gender, MergedRecordFlag, MergedIntoSSID,*/ Prefix, FirstName, MiddleName, LastName, Suffix, AccountID, SSCreatedBy, SSUpdatedBy, SSCreatedDate, SSUpdatedDate, CreatedBy, UpdatedBy, CreatedDate, UpdatedDate
    )
    VALUES (
		mySource.SourceDB
...
THEN UPDATE SET 
    ...
    , myTarget.UpdatedDate = @RunTime
	;
/* Not Needed for MemberProspects -
/***PhoneCell***/
MERGE dbo.DimCustomer AS myTarget
...
WHEN MATCHED ---AND  mySource.SSUpdatedDate >= DateAdd(d, -3, getdate())
...
THEN UPDATE SET 
	...
	--, myTarget.ExtAttribute31 = mySource.ExtAttribute31  ---xml of attributes
