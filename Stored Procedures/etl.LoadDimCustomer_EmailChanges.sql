...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [etl].[LoadDimCustomer_EmailChanges] 
(
	@ClientDB VARCHAR(50),
...
BEGIN
	...
	DECLARE
		...
		, @targetColumns_sql NVARCHAR(MAX) = 
			'''EmailPrimary'',''EmailPrimaryDirtyHash'',''EmailPrimaryIsCleanStatus''
			,''EmailOne'',''EmailOneDirtyHash'',''EmailOneIsCleanStatus''
			,''EmailTwo'',''EmailTwoDirtyHash'',''EmailTwoIsCleanStatus'''
		...
		, @groupID_sql NVARCHAR(MAX) = 
			'CASE 
				WHEN COLUMN_NAME LIKE ''EmailPrimary%'' THEN 1 
				WHEN COLUMN_NAME LIKE ''EmailOne%'' THEN 2
				WHEN COLUMN_NAME LIKE ''EmailTwo%'' THEN 3
			END'
		...
		, @groupDesc_sql NVARCHAR(MAX) = 
			'CASE 
				WHEN COLUMN_NAME LIKE ''EmailPrimary%'' THEN ''Email Primary'' 
				WHEN COLUMN_NAME LIKE ''EmailOne%'' THEN ''Email One''
				WHEN COLUMN_NAME LIKE ''EmailTwo%'' THEN ''Email Two''
			END'
