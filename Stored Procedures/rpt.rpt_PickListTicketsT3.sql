...
--DROP PROC [rpt].[rpt_PickListTicketsT3]
--EXEC  [rpt].[rpt_PickListTickets] '1450'
--EXEC  [rpt].[rpt_PickListTicketsT3] '1450'




CREATE    PROCEDURE [rpt].[rpt_PickListTicketsT3]  (@EventID NUMERIC) WITH RECOMPILE  AS  
...
CREATE TABLE #EventCustomer (cid NUMERIC (38,10), contactID INT
, ADNumber INT, firstname NVARCHAR(150), lastname NVARCHAR(150), fullname NVARCHAR(max), accountnumber NVARCHAR(36))

INSERT INTO #EventCustomer(cid, contactID, ADNumber,firstname,  lastname, fullname, accountnumber)
SELECT DISTINCT 
    c.id AS cid 
	,con.ContactID
	,con.ADNumber
...
CREATE UNIQUE INDEX IDX02 ON #EventCustomer(cid) 
...
CREATE UNIQUE INDEX IDX03 ON #PriorityPoints (ADNumber) 
...
INNER JOIN ods.VTXordergroups og
	ON   a.tixseatordergroupid   = og.ordergroup
INNER JOIN #EventCustomer c  ON og.customerid = c.cid 
INNER JOIN [ods].[VTXtixeventzoneseatgroups] rw 
