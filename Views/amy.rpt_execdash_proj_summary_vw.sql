...
SET ANSI_NULLS ON
GO
CREATE view [amy].[rpt_execdash_proj_summary_vw] as 
select * from (
select case when transyear = 'CAP' then cast(year(transdate) as varchar) else transyear end reportyear, 
...
from advcontacttransheader h, advcontacttranslineitems li, advprogram p
where h.transid = li.transid and li.programid = p.programid
...
group by  case when transyear = 'CAP' then cast(year(transdate) as varchar) else transyear end ,  case when p.CapDriveYear = 1 then 'CAP' else 'Annual' end,
programname) y
GO
