...
SET ANSI_NULLS ON
GO
--exec [amy].[spRandyHowardReport_acct] '3/31/2017', '03/31/2017','2017'
CREATE PROCEDURE [amy].[spRandyHowardReport_bydate]
(
...
sum(case when isnull(sa.premium_ind,0) = 0 then  py.Annual else 0 end) nonpremium_annual_expected 
  ...
  , programid
	 from ( 
		SELECT 	h.ContactID,c.ADNumber,
    format(transdate, 'MM/dd/yyyy') dt,
			l.ProgramID, 
			null LevelName,
   ...
   SUM(CASE when  MatchingGift=0  and l.programid = 674  THEN TransAmount* ISNULL(premium_annual_expected_pct ,1)
      ...
      left join  percenttb on c.adnumber = percenttb.adnumber
		...
		GROUP 	BY h.ContactID, l.ProgramID, p.ProgramName ,c.ADNumber,
    format(transdate, 'MM/dd/yyyy') 
		UNION
		SELECT 	h.ContactID, c.ADNumber,
    format(transdate, 'MM/dd/yyyy') dt,
			l.ProgramID, 
...
sum(case when isnull(sa.premium_ind,0) = 0 then  py.Annual else 0 end) nonpremium_annual_expected 
  ...
  left join seatarea sa on ct.seatareaid = sa.seatareaid
	...
	GROUP BY Allocation, LevelName, programid, dt ) a
  left join ProgramGLMappings pglmap on a.programid = pglmap.programid
 	ORDER BY Allocation,dt
  end
--RETURN
GO
