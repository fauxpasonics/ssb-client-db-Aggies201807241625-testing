...
SET ANSI_NULLS ON
GO
CREATE  procedure [amy].[rpt_fb_seatareapotential_amt_by_section](@ticketyear  int)
 AS
...
case when annual is not null
--isnull([annual] ,0) > 0 
then suiteprice.[annual] else seats.annual_amount end Potential_Annual, Annual_Expected [Net_Annual_Expected],  
original_annual_expected
 [Gross_Annual_Expected],
...
[Season - Non-Renewable],
 --[Individual - Donor], [Inventory], [Student], [Discount],
 [OtherSold],
 [StudentsAvailable], [StudentsComps],[DonorAvailable], [Problem], [Visiting Team], [Athletic Comp], [ADA/Wheelchair], [Obstructed View],[OtherUnsold], [Student Pass],
...
case when annual is not null then suiteprice.[annual] else seats.annual_amount end  +  Ticket_Amount_potential Revenue_Potential, 
...
--end of new 3/21/2018
 --[ods].[VTXtixeventzoneseats] a   --removed 3/21/2017
...
--JOIN [ods].[VTXtixevents] e ON a.tixeventid = e.tixeventid   --removed 3/21/2017
...
isnull(rw.tixseatgroupdesc,'') not in ( 'SRO','Suite Member','ADMIN')  and tixsysseatstatusdesc <>  'Kill'
  ...
  group by  SeatAreaName , seatareaid ,premium_ind , seatsection  --, tixsyspriceleveldesc
  --,
-- case when tixseatcurrentstatus= 2 then 'Sold' else 'Unsold'  end   ,
 ...
 left join (  
  select seatareaname, seatareaid, seatsection, sum( adjusted_annual)   Annual_Expected , sum(annual_expected ) original_annual_expected , 
 sum(creditapplied) creditapplied,
 sum(FSLMCredit) FSLMCredit from (
 select  --tt.adnumber,
 tt.seatareaid, seatareaname, premium_ind,annual_expected , seatsection, isnull( credit, 0) creditapplied , isnull(FSLMCredit,0) FSLMCredit,
 case when  isnull( credit, 0) <>0 then annual_expected -  isnull( credit, 0) else annual_expected end adjusted_annual
from (
select  -- adnumber, 
seatareaname, seatareaid , premium_ind, seatsection, sum(annual_expected) annual_expected , sum(credit) credit,  sum(FSLMCredit) FSLMCredit
-- ,  sum(nonpremium_annual_expected )  nonpremium_annual_expected 
from (
select accountnumber adnumber, sa.seatareaname, sa.seatareaid , premium_ind, seatsection,
 ...
 seatarea sa 
  on ct.seatareaid = sa.seatareaid
      ...
      py.sporttype =  'FB'  and py.ticketyear = @ticketyear
          ...
          group by accountnumber, sa.seatareaname, premium_ind, sa.seatareaid,  seatsection
union  all
select adnumber, sa.seatareaname, sa.seatareaid , premium_ind,  Suite,
...
group by  adnumber, sa.seatareaname, sa.seatareaid , premium_ind, suite
union all
select accountnumber adnumber, sa.seatareaname, sa.seatareaid , premium_ind, seatsection,
...
group by -- adnumber,
seatareaname, seatareaid , premium_ind ,seatsection
          ) tt    -- on sarea.seatareaid = tt.seatareaid
        /*  left   join  (
          /*select adnumber, sum(transamount) creditapplied from advcontact c, 
       advcontacttransheader h       , advcontacttranslineitems l where c.contactid = h.contactid and  h.transid = l.transid and transyear = '2017'
       and transtype = 'Credit' and programid = 678
       group by adnumber
       */
       select-- adnumber, 
       seatareaid, sum(credit) credit from (
         select adnumber,  seatareaid  ,  sum(credit) credit from EndowedCreditsbyYear  u where renewalyear = 2017 and driveyear = '2017' and CreditType = 'Annual'
        group by adnumber, seatareaid
                union 
        select adnumber , salloc.seatareaid, sum(transamount) credit from   amy.SeatAllocation salloc, adv_transaction_vw  v where
         salloc.programid= v.programid and transyear = '2017' 
        and transtype = 'Credit' and  programtypecode ='ADAAC'
        group by adnumber, seatareaid 
        union 
           select adnumber, (  select   min(seatareaid ) seatareaid from seatdetail_individual_history where 
  accountnumber  = v.adnumber and tixeventlookupid = 'F17-Season' and cancel_ind is null and seatpricecode not like 'Comp%') seatareaid, 
      sum(transamount) Credit from adv_transaction_vw v 
      where transtype = 'Credit' and transyear = '2017'
      ...
      and programid in (select programid from seatallocation where  sporttype = 'FB')
  group  by  adnumber
    having sum(transamount) <>0 
        ...
        ) Credit on  tt.seatareaid = credit.seatareaid  */
        ) annualamt
        group by seatareaname, seatareaid, seatsection  ) annual_exp on  annual_exp.seatareaid = seats.seatareaid and
        annual_exp.seatsection = seats.seatsection
        left join (select seatareaid, sectionnum,
        sum(annual) annual from suitepricing where cast(@ticketyear as varchar) between startyear and endyear
        group by seatareaid, sectionnum) suiteprice on    suiteprice.seatareaid = seats.seatareaid  and   suiteprice.sectionnum = seats.seatsection
