...
SET ANSI_NULLS ON
GO
create view [amy].[eventtickets2] as
select c.accountnumber, te.tixeventid, tixeventlookupid,pc.tixsyspricecodecode, pc.tixsyspricecodedesc seatpricecode,  --sum(oi.paid_amount ) paidamount,
  sg2.tixseatgroupdesc Neighborhood,    sg1.tixseatgroupdesc seatsection, 
  sg.tixseatgroupdesc seatrow   , --   number4   -- ,seat.tixseatdesc --
  count(  oi.number4) qty -- count(isnull(seat.tixseatdesc,1)) qty
   from  ods.VTXtixevents te  
...
where  --te.tixeventlookupid = 'B15-MB'  and
 --te.tixeventid = 3412 and -- og.ordergroup = 27965547
