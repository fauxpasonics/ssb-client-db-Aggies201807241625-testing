...
SET ANSI_NULLS ON
GO
--exec amy.rpt_capstatement_detail 21426,2017
CREATE procedure [amy].[rpt_capstatement_detail]
(@adnumber int, 
@ticketyear int)
as
select c.adnumber, c.accountname, getdate() "OrderDate",
...
, 
tall.*,
case when pledge_amount +sr.[Adv CAP Credit]<> 0  then (isnull(cast(receipt_amount as decimal),0) +sr.[Adv CAP Credit]) /(pledge_amount+sr.[Adv CAP Credit]) else 0 end percentpaid,
sr.CAPCredit, sr.CAP, sr.cap_amount, sr.seatregionname, sr.seatregionid, sr.[Ticket Total], 
sr.cap_20, sr.CAP_40, sr.CAP_60, sr.CAP_80, sr.CAP_100, sr.CAP_OTHER, sr.CAP_DUE, sr.CapStillDue,
sr.[Adv CAP Credit] CapCredit,
isnull(cast(pledge_amount as decimal),0) - isnull(cast(receipt_amount as decimal),0)- isnull(cast(sr.[Adv CAP Credit]  as decimal),0)  balance,
isnull(cast(receipt_amount as decimal),0) +sr.[Adv CAP Credit]   receipt_credit
from (
select contactid,  
...
--and balance > 0
--and pledge_amount - receipt_amount- CapCredit 
/*
...
and programgroup = 'Kyle Field Campaign'  
order by adnumber --, programgroup, tall.programname


/****** Object: Procedure [amy].[rpt_capstatement_header]   Script Date: 8/26/2016 11:43:29 AM ******/
/****** Object: Procedure [amy].[rpt_capstatement_header]   Script Date: 8/26/2016 11:43:29 AM ******/
/****** Object: Procedure [amy].[rpt_capstatement_header]   Script Date: 8/26/2016 11:43:29 AM ******/
/****** Object: Procedure [amy].[rpt_capstatement_header]   Script Date: 8/26/2016 11:43:29 AM ******/
/****** Object: Procedure [amy].[rpt_capstatement_header]   Script Date: 8/26/2016 11:43:29 AM ******/
