...
SET ANSI_NULLS ON
GO
CREATE  view [amy].[rpt_matching_aging_vw] as
...
SELECT  sumrec.*,
  ...
  (select h1.contactid, adnumber,accountname, transdate,transyear,li1.programid,  p1.programname,
     sum( case when transtype like '%Pledge%' then MatchAmount else 0 end) pledge , 
     sum( case when transtype like '%Receipt%' then MatchAmount else 0 end) receipt
     FROM advcontact c1,  
     ...
     (matchinggift = 0 OR matchinggift IS NULL) and matchamount <> 0 
     group by h1.contactid, adnumber,accountname,  transdate,transyear,li1.programid,p1.programname
     ) SumRec
) t
  where pledgetotal>receipttotal   and RunningPledgeTotal >ReceiptTotal and pledge >0 
  ) l where balance > 1
GO
