...
SET ANSI_NULLS ON
GO
--select * into RPT_SUITESEATREGION_TB_bk05152018 from RPT_SUITESEATREGION_TB
-- exec [amy].[proc_SUITESEATREGION_table]  'FB', '2018'
CREATE PROCEDURE [amy].[proc_SUITESEATREGION_table] (@sporttype varchar(20) = 'FB', @ticketyear varchar(4)= '2016')
AS


Declare @tixeventlookupid varchar(50)
Declare @pktixeventlookupid varchar(50)
Declare @tixeventid int
Declare @tk_report_tixeventlookupid varchar(50)


select @tixeventlookupid= tixeventlookupid, @pktixeventlookupid = pktixeventlookupid, @tixeventid = tixeventid
from amy.PriorityEvents pe1 where sporttype = @sporttype and ticketyear = @ticketyear

if @tixeventid is null
begin
 set @tk_report_tixeventlookupid = (select tixeventlookupid from amy.PriorityEvents pe1 where sporttype = @sporttype and ticketyear = @ticketyear-1);
end
else
begin
    set @tk_report_tixeventlookupid = @tixeventlookupid
end


delete from RPT_SUITESEATREGION_TB  where sporttype = @sporttype and ticketyear = @ticketyear

insert into RPT_SUITESEATREGION_TB
select  
null percentdue, suiteinfo.adnumber adnumber,
(select accountname from advcontact where adnumber = suiteinfo.adnumber) accountname, null status,
(select sr.seatregionname from seatregion sr,  seatarea  sa where sr.seatregionid = sa.seatregionid and sa.seatareaid = suiteinfo.seatareaid ) + ' - '+ suite seatregionname, 
...
WHERE resp_code IS NULL AND resp_text IS NULL 
 AND t.contactid  in (select contactid from  advcontact  c where adnumber = SUITEinfo.adnumber )
...
isnull( (SELECT sum (transamount) ScheduledPayments
         ...
         WHERE     ps.ProgramID = sa1.ProgramId
               ...
               AND ps.contactid   in (select contactid from  advcontact  c where adnumber = SUITEinfo.adnumber )
               and sa1.sporttype =  @sporttype
               ), 0) 
...
round(CAPCOMBAMOUNTEXPECTED,0) CAP_DUE, 
null LinealTransfer, 
case when CAPCOMBAMOUNTEXPECTED -isnull(capital_receipt_trans_amount,0) - isnull(capital_pledge_match_amount,0)  -  isnull(cap_credit_amount,0)  < 0 then 0 else
round((CAPCOMBAMOUNTEXPECTED -isnull(capital_receipt_trans_amount,0) - isnull(capital_pledge_match_amount,0)  -  isnull(cap_credit_amount,0)) ,0) end CapStillDue,
ordernumber, 
seatlastupdated , 
null vtxcusttype,
suite , email , cappercent, ordergroupbottomlinegrandtotal, ordergrouptotalpaymentscleared, ordergrouptotalpaymentsonhold,
STUFF((
       SELECT  ',' + seatsection  FROM  seatdetail_individual_history z where tixeventlookupid = @pktixeventlookupid and seatpricecode like 'Suite%' and cancel_ind is null
       and z.accountnumber = suiteinfo.adnumber
       order by seatsection
       FOR XML	PATH('')
       ), 1, 1, '' )    Parking  
from 
(select @sporttype Sporttype, @ticketyear Ticketyear, sp.sectionnum, sp.CAPTOTAL, sp.annual , sp.seatareaid, t.* 
from suitepricing sp,
(
 select  SUITEACCT.suite ,   SUITEACCT.adnumber,
...
sum(cap_other) CAP_OTHER  , 
 email,MAX(CAPPERCENT) cappercent
 from
(select  suite ,  adnumber,
         ...
         max(case when donationtype = 'CAP' then percentexpected else null end) cappercent
 ...
 left join ( select salloc.suite,  p.acct adnumber, -- null contactid,
 e.PHONE email,
...
from PacTranItem_alt_vw  p
  left join dbo.PD_PATRON_PHONE_TYPE e on p.acct =  e.PATRON and phone_type = 'E'
...
group by salloc.suite ,p.acct,  e.phone
--having sum( isnull(l.transamount,0)+ isnull(l.matchamount,0)) <> 0 
) amounts
on amounts.suite = SUITEACCT.suite and  amounts.adnumber = SUITEACCT.adnumber
group by SUITEACCT.suite, suiteacct.adnumber,  email
) t where sp.sectionnum = t.suite ) suiteinfo
left join 
 (
select  sporttype,
     adnumber,   seatsection ,  
    sum(ADA) ADA,
    sum(E) E, 
    sum(EC) EC, 
    sum("ZC E") "ZC E",
   ...
   -- sum(CAP_DUE ) CAP_DUE  ,
       ...
       max(ordernumber) ordernumber,  count(distinct ordernumber) ordernumbercount, 
        ...
        from  (
     select  seatsection , isnull(py.sporttype, @sporttype) sporttype, min(renewal_date) renewal_date, cast(accountnumber as int)  adnumber, 
    ...
    sum(sapg.CAP * ct.CAP_Percent_Owe_ToDate ) CAP_DUE  ,
     ...
     from amy.seatdetail_individual_history  ct 
         left join amy.PriceCodebyYear py on ( py.sporttype =  @sporttype  and py.ticketyear =  @ticketyear 
          ...
          where  ct.tixeventlookupid =  @tk_report_tixeventlookupid    and cancel_ind is null  
          and ct.seatareaid in (select seatareaid from seatarea where seatingtype = 'Suite')
       group by     seatsection, 
       isnull(py.sporttype,@sporttype),        cast( accountnumber as int), ct.paidpercent  , 
       ...
       )  orderlevel group by adnumber, sporttype    , seatsection  
       ) ticketing  on   suiteinfo.adnumber = ticketing.adnumber and suiteinfo.suite = ticketing.seatsection
GO
