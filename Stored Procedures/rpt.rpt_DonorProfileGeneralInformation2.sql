...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [rpt].[rpt_DonorProfileGeneralInformation2] (@ADNumber VARCHAR(300))/*, @FirstName varchar(300)='', @LastName varchar(300)='')*/
WITH RECOMPILE
AS
...
IF (COALESCE(@ADNumber,'NONESUPPLIED')='NONESUPPLIED')
BEGIN
...
SELECT TOP 100
	...
	,REPLACE(c.AccountName,'&','&amp;') AS AccountName
	...
	,REPLACE(c.UDF4 , '&','&amp;') AS MajorGiftsNamingOpp
